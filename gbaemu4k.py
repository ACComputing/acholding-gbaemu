...
class ARM7TDMI:
    ...
    def execute_arm(self, instr):
        ...
        self.halted = True
        self.halt_reason = f"Unimplemented ARM: 0x{instr:08X} at 0x{self.r[15]-4:08X}"
    ...
    def execute_thumb(self, instr):
        ...
        self.halted = True
        self.halt_reason = f"Unimplemented THUMB: 0x{instr:04X} at 0x{self.r[15]-2:08X}"
...
class GBAMemoryBus:
    ...
    def _map(self, addr):
        ...
        if 0x08 <= region <= 0x0D: return self.rom, addr & 0x1FFFFFF
        return None, 0
